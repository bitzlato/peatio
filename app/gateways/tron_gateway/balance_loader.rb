  ...
  module BalanceLoader
    ...
    def load_balance(address, currency)
      ...
      if contract_address.present?
        load_trc20_balance(address, contract_address)
      else
        load_basic_balance(address)
      end.yield_self { |amount| blockchain_currency.to_money_from_units(amount.to_i) }
    ...
    def load_basic_balance(address)
      client.json_rpc(path: 'wallet/getaccount',
                      params: { address: reformat_decode_address(address) }).fetch('balance', nil).to_i
    end
  end
end
