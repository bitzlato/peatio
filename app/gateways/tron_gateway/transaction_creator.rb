  ...
  module TransactionCreator
    def create_transaction!(from_address:,
      ...
      if contract_address.present?
        args.merge!(contract_address: contract_address, fee_limit: (fee_limits[contract_address] || TronGateway::FEE_LIMIT))
    ...
    def create_coin_transaction!(from_address:, to_address:, amount:, private_key:)
      ...
      txid = response.dig('transaction', 'txID').yield_self { |id| reformat_txid(id) }
