      ...
      def self.process(job_timeout = JOB_TIMEOUT)
        ...
        Blockchain.active.find_each do |blockchain|
          ...
          if gas_price < max_gas_price && !blockchain.high_transaction_price_at.nil?
            ...
            Peatio::SlackNotifier.instance.ping "Включил выводы для #{blockchain.name}. Цена газа #{gas_price * 1e-9} Gwei ниже, чем #{max_gas_price * 1e-9} Gwei."
            ...
            Peatio::SlackNotifier.instance.ping "Отключил выводы для #{blockchain.name}. Цена газа #{gas_price * 1e-9} Gwei выше, чем #{max_gas_price * 1e-9} Gwei."
          end
        end
