...
module Matching
  class Engine
    ...
    def publish_increment(market, side, price, amount)
      ...
      # Publish snapshot if:
      # last snapshot time > 1 min
      # increment_count > 20 and last snapshot time > 10 second
      if (Time.now - @snapshot_time > MAX_PERIOD_TO_SNAPSHOT) ||
          (@increment_count >= MIN_INCREMENT_COUNT_TO_SNAPSHOT && (Time.now - @snapshot_time > MIN_PERIOD_TO_SNAPSHOT))
        publish_snapshot
        @increment_count = 0
      end
      @increment_count += 1
