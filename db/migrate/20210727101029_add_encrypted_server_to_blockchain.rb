class AddEncryptedServerToBlockchain < ActiveRecord::Migration[5.2]
  def up
    ...
    remove_column :blockchains, :server if column_exists?(:blockchains, :server)
    add_column :blockchains, :server_encrypted , :string, limit: 1024, after: :client unless column_exists?(:blockchains, :server_encrypted)
  ...
  def downcase
    ...
    add_column :blockchains, :server, :string, limit: 1000, default: '', null: false, after: :client unless column_exists?(:blockchains, :server)
    remove_column :blockchains, :server_encrypted , :string, limit: 1024, after: :client if column_exists?(:blockchains, :server_encrypted)
