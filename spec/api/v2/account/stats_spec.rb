...
describe API::V2::Account::Stats, type: :request do
  ...
  describe 'GET /api/v2/account/stats/pnl' do
    ...
    let!(:pnl1) do create(:stats_member_pnl, pnl_currency_id: eth.id, currency_id: btc.id,
                               total_credit: 0.1, total_credit_fees: 0.01, total_debit_fees: 0.02, total_credit_value: 0.3, total_debit: 0.2,
                               total_debit_value: 10.0, average_balance_price: 0.42, member: member) end

    let!(:pnl2) do create(:stats_member_pnl, pnl_currency_id: btc.id, currency_id: eth.id,
                                total_credit: 0.1, total_credit_fees: 0.01, total_debit_fees: 0.02, total_credit_value: 0.3, total_debit: 0.2,
                                total_debit_value: 10.0, average_balance_price: 0.21, member: member) end
    ...
    context 'avarage sell price equal to 0' do
      ...
      let!(:pnl) do create(:stats_member_pnl, pnl_currency_id: usd.id, currency_id: btc.id,
                                  total_credit: 0.1, total_credit_fees: 0.01, total_debit_fees: 0.0, total_credit_value: 0.3, total_debit: 0.0,
                                  total_debit_value: 0.0, average_balance_price: 0.12, member: member) end
