...
RSpec.describe CurrencyServices::SwapPrice do
  ...
  let(:deviation) { 0.02 }
  let(:top_price) { 10.to_d * market.price_precision }

  context 'sale btc for usd' do
    subject { described_class.new(from_currency: from_currency, to_currency: to_currency, request_volume: request_volume) }

    let(:request_volume) { 3 }
    let(:price) { market.round_price(top_price - (top_price * deviation)) }
    let(:inverse_price) { market.round_price(1 / price) }

    before { OrderBid.stubs(:top_price).returns(top_price) }
    ...
    it { expect(subject.request_volume).to eq(request_volume) }
    it { expect(subject.volume).to eq(request_volume) }
  ...
  context 'buy btc for usd' do
    subject { described_class.new(from_currency: to_currency, to_currency: from_currency, request_volume: request_volume) }
    ...
    before { OrderAsk.stubs(:top_price).returns(top_price) }
    ...
    it { expect(subject.request_volume).to eq(request_volume) }
    it { expect(subject.volume).to eq(volume) }
